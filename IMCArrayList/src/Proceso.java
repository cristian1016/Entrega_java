...
public void actualizacionPesoYEstatura(Double peso, Double talla) {
    String consulta1 ="";
    System.out.println(nombres+"\n");
    consulta1 = JOptionPane.showInputDialog("Ingrese el nombre del paciente a actualizar");

    if(nombres.contains(consulta1)) {
        for (int i = 0; i < nombres.size(); i++) {
            if(nombres.get(i).equalsIgnoreCase(consulta1)) {
                peso = Double.parseDouble(JOptionPane.showInputDialog("Ingrese peso del paciente"));
                talla = Double.parseDouble(JOptionPane.showInputDialog("Ingrese la estatura del paciente"));
                
                this.peso = peso; // Actualizar peso en resultados
                this.talla = talla; // Actualizar talla en resultados
                imprimirResultados();

                System.out.println("El peso y estatura se han actualizado correctamente");
            }
        }
    } else {
        System.out.println("El paciente no existe");
	...
	/*Intente actualizar peso y estatura pero no logre como, quisiera saber como pude haber solucionado */
}
